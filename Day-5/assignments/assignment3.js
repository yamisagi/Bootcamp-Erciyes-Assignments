...
const showResult = (filtered) => {
  if (filtered.length === 0) {
    result.innerHTML = '<h2>No Products Found</h2>';
    return;
  }
  let html = filtered
    .map((product) => {
      ...
      <li class="card">
        ...
        <p
        style="color: ${product.unitsInStock === 0 ? 'red' : 'black'}"
        >Stock: ${product.unitsInStock}</p>
      </li>
      `;
    })
    .join('');
...
searchBtn.addEventListener('click', (e) => {
  ...
  let filtered = searchMinMax(min, max);
  showResult(filtered);
});
